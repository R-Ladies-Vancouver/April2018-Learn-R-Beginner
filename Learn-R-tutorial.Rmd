...
There are several functions that will provide information about the structure of the dataframe: `str()`, `head()` (part of base R) or `glimpse()`(part of DPLYR package), which extracts the top few rows of a dataset and print it in the console. This kind of data inspecting is handy when we are working with large datasets (otherwise R would try to print the entire data!). Note the differences in the output in your R console when you run these three functions.
...
The `select` function allows to extracts specific variables from a dataset (dataframe). If we then store variables of interest we in an object, we can focus on working on a small subset of the data seperately.
...
We use the `summarize()` in conjunction with `group_by`to perform calculations / summarize data based on the categories of interest within the dataset. This is where we can make a use of base R functions we explored at the beginning of the workshop.

An example: What is the average age of people who survived?
Steps: 

1. Group your data by variable **Survived**
...
The average age of people who survived is 28.3
